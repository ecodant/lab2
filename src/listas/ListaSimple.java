...
public class ListaSimple<T> implements Iterable<T> {
	...
	public T eliminar(T dato) {
		...
		// buscar el nodo y su previo
		while (nodo != null) {
			if (nodo.getValorNodo().equals(dato)) { // Fixed: use equals instead of == for object comparison
				encontrado = true;
				break;
			}
			previo = nodo;
		...
		if (encontrado) {
			...
			if (siguiente == null) {
				nodoUltimo = previo; // Fixed: update nodoUltimo when removing the last element
			} else {
	...
	public T eliminarPrimero() {
		if (!estaVacia()) {
			...
			if (nodoPrimero == null) {
				nodoUltimo = null; // Fixed: update nodoUltimo when list becomes empty
			}
